...
/**
 * Scenario 1: Small Files Test
 *
 * 5 users uploading small files (100-500 rows)
...
async function uploadFile(filePath, userNumber, userToken) {
  ...
  try {
    ...
    while (status !== 'Completed' && status !== 'Failed' && pollCount < 60) {
      ...
      if (pollCount % 5 === 0) {
        console.log(`⏳ [User ${userNumber}] Status: ${status} (${pollCount}s elapsed)`);
      }
    }
...
async function runTest() {
  console.log(`\n${'='.repeat(80)}`);
  console.log(`🧪 SCENARIO 1: SMALL FILES TEST`);
  console.log(`   ${CONCURRENT_USERS} users uploading small files (100 rows each)`);
  ...
  console.log(`Total test time: ${((metrics.endTime - metrics.startTime) / 1000).toFixed(2)}s\n`);
  ...
  console.log(`\n✅ Expected: All uploads complete quickly with minimal queuing`);
  console.log(`${'='.repeat(80)}\n`);
}
