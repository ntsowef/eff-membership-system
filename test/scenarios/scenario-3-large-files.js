...
async function uploadFile(filePath, userNumber, userToken) {
  ...
  try {
    ...
    while (status !== 'Completed' && status !== 'Failed' && pollCount < 300) {
      ...
      if (pollCount % 10 === 0) {
        console.log(`⏳ [User ${userNumber}] Status: ${status} (${pollCount * 3}s elapsed)`);
      }
    }
...
async function runTest() {
  console.log(`\n${'='.repeat(80)}`);
  console.log(`🧪 SCENARIO 3: LARGE FILES TEST`);
  console.log(`   ${CONCURRENT_USERS} users uploading large files (5000-10000 rows)`);
  ...
  if (metrics.uploads.length > 0) {
    ...
    console.log(`Average processing time: ${(avgProcessingTime / 1000 / 60).toFixed(2)} minutes`);
    console.log(`Average total time: ${(avgTotalTime / 1000 / 60).toFixed(2)} minutes`);
  }
  
  console.log(`\n✅ Expected: Queue handles long-running jobs, no timeouts`);
  console.log(`${'='.repeat(80)}\n`);
}
