#!/usr/bin/env python3
"""
Safely delete ward codes containing 'JHB' from the wards table
...
print("=" * 80)
print("SAFE DELETE: WARDS CONTAINING 'JHB'")
print("=" * 80)
...
try:
    ...
    if user_references:
        ...
        for user_id, name, email, ward_code in user_references:
            print(f"  - User ID {user_id} ({name} - {email}): ward_code = {ward_code}")
    else:
    ...
    total_references = len(user_references) + member_count + old_member_count
    ...
    print(f"4. Delete {jhb_ward_count} JHB wards from wards table")
    print(f"\nTotal records to update: {total_references}")
    
    # Confirm deletion
    print("\n" + "=" * 80)
    user_input = input(f"Type 'DELETE' to proceed with deletion: ")
    ...
    # Step 6: Update members_consolidated table
    if member_count > 0:
        print("\nStep 6: Updating members_consolidated table...")
        cursor.execute("""
    ...
    # Step 7: Update members table (old)
    if old_member_count > 0:
        print("\nStep 7: Updating members table (old)...")
        cursor.execute("""
            UPDATE members 
            SET ward_code = NULL 
            WHERE ward_code IN (SELECT ward_code FROM wards WHERE ward_code LIKE '%JHB%')
    ...
    # Step 8: Delete JHB wards
    print("\nStep 8: Deleting JHB wards...")
