 ...
 * 
 * This script tests all email templates in the EFF Membership System
 * 
 * Usage:
 *   npm run test:email -- --email your-email@example.com
 *   or
 *   ts-node test/email/email-templates-test.ts --email your-email@example.com
 * 
 * Options:
...
const templateTests: TemplateTest[] = [
  {
    name: 'welcome-email',
...
function parseArgs(): { email: string; template?: string; all: boolean } {
...
${colors.bright}Environment Variables Required:${colors.reset}
...
function displaySummary(results: { name: string; success: boolean; error?: string }[]) {
