...
const MembersListModal: React.FC<MembersListModalProps> = ({
  ...
  // Fetch members for this voting district
  const { data: membersData, isLoading, error } = useQuery({
    queryKey: ['members-by-voting-district', votingDistrictCode, page],
    queryFn: () => searchApi.getMembersByVotingDistrict(votingDistrictCode, {
      page,
      limit
    }),
    enabled: open && !!votingDistrictCode,
  });
    ...
    >
        ...
        <Box display="flex" justifyContent="space-between" alignItems="center">
          <Box>
            <Typography variant="h6" component="div">
              Members in Voting District
            </Typography>
            <Typography variant="body2" color="text.secondary">
              {votingDistrictName} ({votingDistrictCode}) - {memberCount} members
            </Typography>
            {districtInfo && (
              <Typography variant="caption" color="text.secondary" display="block">
                {districtInfo.ward_name}, {districtInfo.municipal_name}, {districtInfo.district_name}, {districtInfo.province_name}
              </Typography>
            )}
          </Box>
      ...
      <DialogContent dividers>
        ...
        {!isLoading && !error && members.length === 0 && (
          <Alert severity="info">
            No members found in this voting district.
          </Alert>
        )}
