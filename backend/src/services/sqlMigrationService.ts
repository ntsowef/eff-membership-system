...
export class SQLMigrationService {
  ...
  private static convertComplexMySQLQuery(query: string): string {
    ...
    // Handle LPAD function - cast first argument to TEXT if it's a column reference
    convertedQuery = convertedQuery.replace(
      /LPAD\(([^,]+),\s*(\d+),\s*'([^']+)'\)/g,
      (match, arg1, arg2, arg3) => {
        // If arg1 looks like a column reference (contains . or is alphanumeric), cast it to TEXT
        const trimmedArg1 = arg1.trim();
        if (/^[a-zA-Z_][a-zA-Z0-9_.]*$/.test(trimmedArg1)) {
          return `LPAD(${trimmedArg1}::TEXT, ${arg2}, '${arg3}')`;
        }
        return `LPAD(${trimmedArg1}, ${arg2}, '${arg3}')`;
      }
    );
    ...
    // Use a more sophisticated approach to handle nested functions
    // We need to manually find CONCAT calls and extract their full content
    let concatMatches: Array<{ start: number; end: number; content: string }> = [];
    let searchPos = 0;

    while (true) {
      const concatIndex = convertedQuery.indexOf('CONCAT(', searchPos);
      if (concatIndex === -1) break;

      // Find the matching closing parenthesis
      let parenDepth = 1;
      let endIndex = concatIndex + 7; // Start after 'CONCAT('

      while (endIndex < convertedQuery.length && parenDepth > 0) {
        if (convertedQuery[endIndex] === '(') {
          parenDepth++;
        } else if (convertedQuery[endIndex] === ')') {
          parenDepth--;
        }
        endIndex++;
      }
      ...
      if (parenDepth === 0) {
        // Extract the arguments (everything between CONCAT( and the matching ))
        const argsContent = convertedQuery.substring(concatIndex + 7, endIndex - 1);
        concatMatches.push({
          start: concatIndex,
          end: endIndex,
          content: argsContent
    ...
    for (let i = concatMatches.length - 1; i >= 0; i--) {
      const match = concatMatches[i];
      ...
      for (let j = 0; j < match.content.length; j++) {
        const char = match.content[j];
        ...
        if (char === '(') {
          ...
          // This is a top-level comma, so it's an argument separator
      ...
      // Replace the CONCAT call with the PostgreSQL equivalent
      const replacement = argList.join(' || ');
      convertedQuery = convertedQuery.substring(0, match.start) + replacement + convertedQuery.substring(match.end);
    }

    // Handle DATEDIFF function (MySQL to PostgreSQL)
    ...
    convertedQuery = convertedQuery.replace(
      ...
      (match, date1, date2) => {
        ...
        return `EXTRACT(DAY FROM (${cleanDate1})::TIMESTAMP - (${cleanDate2})::TIMESTAMP)::INTEGER`;
      }
    );
    ...
    convertedQuery = convertedQuery.replace(
      /DATE_SUB\(([^,]+),\s*INTERVAL\s+(\$\d+)\s+(\w+)\)/g,
      (match, date, param, unit) => {
        const pgUnit = unit.toLowerCase().replace(/s$/, ''); // Remove plural 's'
    ...
    convertedQuery = convertedQuery.replace(
      /TIMESTAMPDIFF\(\s*MONTH\s*,\s*([^,]+),\s*COALESCE\(([^,]+),\s*CURRENT_TIMESTAMP\)\)/gi,
    ...
    convertedQuery = convertedQuery.replace(
      /TIMESTAMPDIFF\(\s*MONTH\s*,\s*([^,]+),\s*([^)]+)\)/gi,
      (match, startDate, endDate) => {
        const cleanStart = startDate.trim();
        const cleanEnd = endDate.trim();
    ...
    convertedQuery = convertedQuery.replace(
      /TIMESTAMPDIFF\(\s*MINUTE\s*,\s*([^,]+),\s*([^)]+(?:\([^)]*\))*[^)]*)\)/gi,
      (match, startDate, endDate) => {
        const cleanStart = startDate.trim();
        const cleanEnd = endDate.trim();
    ...
    convertedQuery = convertedQuery.replace(
      /TIMESTAMPDIFF\(\s*SECOND\s*,\s*([^,]+),\s*([^)]+(?:\([^)]*\))*[^)]*)\)/gi,
      (match, startDate, endDate) => {
        const cleanStart = startDate.trim();
        const cleanEnd = endDate.trim();
    ...
    const nullifPlaceholders: string[] = [];
    ...
    convertedQuery = convertedQuery.replace(
      /ORDER BY\s+([a-zA-Z_][a-zA-Z0-9_]*)\s+(ASC|DESC)/gi,
      (match, columnAlias, direction) => {
    ...
    if (needsOrderByFix && convertedQuery.includes('GROUP BY')) {
      ...
      while (i < convertedQuery.length && /\s/.test(convertedQuery[i])) i++;
    ...
    convertedQuery = convertedQuery.replace(
      /HAVING\s+([a-zA-Z_][a-zA-Z0-9_]*)\s*([><=!]+)\s*(\d+)/gi,
      (match, columnAlias, operator, value) => {
    ...
    if (convertedQuery.includes('CASE') && convertedQuery.includes('as age_group')) {
      ...
      if (caseExpressionMatch) {
        ...
        convertedQuery = convertedQuery.replace(
          /ORDER BY\s+CASE\s+age_group[\s\S]*?END/gi,
          (match) => {
