...
-- Migration: 023_financial_dashboard_summary_tables_gui.sql
-- Purpose: Create optimized summary tables for financial dashboard performance
--          GUI-Compatible Version (no \echo commands)
-- PostgreSQL Version
-- =====================================================
...
-- Note: These will fail if unified_financial_transactions is a view (expected)
...
SELECT 'Migration completed successfully!' as status,
       (SELECT COUNT(*) FROM daily_financial_summary) as daily_summary_count,
       (SELECT COUNT(*) FROM monthly_financial_summary) as monthly_summary_count,
       (SELECT COUNT(*) FROM financial_reviewer_performance) as reviewer_performance_count,
       (SELECT COUNT(*) FROM financial_dashboard_cache) as cache_count,
       (SELECT COUNT(*) FROM financial_kpi_tracking) as kpi_count;
