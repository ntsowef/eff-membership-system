-- ============================================================================
-- Suspend automatic transition to "Expired" (status_id = 2) until 2026-11-04
-- ============================================================================
-- Business decision: For members whose memberships are expiring between today
-- and 5 November 2026, do NOT let the auto-status mechanism move them to
-- Expired (status_id = 2). Until and including 2026-11-04, any row that would
-- otherwise be set to Expired is kept in Grace Period (status_id = 7).
--
-- From 2026-11-05 onwards the original behaviour resumes automatically (no
...
DECLARE
  ...
  v_expired_suspend_until CONSTANT DATE := DATE '2026-11-04';
BEGIN
...
'Auto-updates membership_status_id based on expiry_date. The "→ Expired" branch is suspended until 2026-11-04 (inclusive); affected rows are kept in Grace Period until then. Manual statuses (Suspended, Cancelled, Pending) are never overridden.';

COMMENT ON FUNCTION fn_auto_update_membership_status() IS
'Trigger function for membership status auto-update. Rules: Active (>=today), Grace Period (0-90 days past expiry), Expired (>90 days past expiry, but suspended until 2026-11-04), Inactive (null expiry).';
...
-- (i.e. NOT in Suspended/Cancelled/Pending). After 2025-11-04 the regular
...
DECLARE
  v_moved INT := 0;
BEGIN
  IF CURRENT_DATE <= DATE '2026-11-04' THEN
...
DO $$
BEGIN
  RAISE NOTICE '✅ Membership status trigger updated: "→ Expired" suspended until 2026-11-04 (inclusive).';
END $$;
