...
async function createSimpleVotingDistrictViews() {
  let connection;
  
  try {
    console.log('🔄 Creating simplified voting district views...');
    ...
    const [tables] = await connection.execute(`
      ...
      AND TABLE_NAME IN ('genders', 'races', 'citizenships', 'languages', 'occupations', 'qualifications', 'voter_statuses')
    `);
    
    const existingTables = tables.map(t => t.TABLE_NAME);
    console.log('📋 Existing lookup tables:', existingTables.join(', '));
    ...
    // 2. Create main members with voting districts view (simplified)
    console.log('📊 Creating simplified members_with_voting_districts view...');
    await connection.execute(`
      ...
      SELECT 
        ...
        m.date_of_birth,
        m.gender_id,
        ${existingTables.includes('genders') ? 'g.gender_name,' : 'NULL as gender_name,'}
      ...
      -- Only join tables that exist
      ${existingTables.includes('genders') ? 'LEFT JOIN genders g ON m.gender_id = g.id' : ''}
    `);
...
createSimpleVotingDistrictViews();
