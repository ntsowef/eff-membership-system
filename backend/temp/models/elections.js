...
class ElectionModel {
    ...
    static async getElections(limit = 20, offset = 0, filters = {}) {
        try {
    ...
    static async getElectionById(id) {
      ...
      `;
            ...
            throw (0, errorHandler_1.createDatabaseError)('Failed to get election', error);
        }
    }
    // Create election
    ...
    static async updateCandidate(id, updateData) {
        try {
            ...
            throw (0, errorHandler_1.createDatabaseError)('Failed to update candidate', error);
        }
    }
    // Cast vote
    ...
    static async getElectionResults(electionId) {
        ...
        WHERE le.id = ?
      `;
            const election = await (0, database_1.executeQuerySingle)(electionQuery, [electionId]);
            if (!election)
                return null;
      ...
      `;
            const candidates = await (0, database_1.executeQuery)(candidatesQuery, [
    ...
    static async finalizeElection(electionId) {
        ...
        LIMIT 1
      `;
            const winner = await (0, database_1.executeQuerySingle)(winnerQuery, [electionId]);
            if (!winner) {
            ...
            // Update election status
            await (0, database_1.executeQuery)('UPDATE leadership_elections SET election_status = "Completed", updated_at = CURRENT_TIMESTAMP WHERE id = ?', [electionId]);
            return true;
        }
        catch (error) {
    ...
    static async canMemberVote(electionId, memberId) {
        try {
            // Check if member has already voted
            const existingVote = await (0, database_1.executeQuerySingle)('SELECT id FROM election_votes WHERE election_id = ? AND voter_member_id = ?', [electionId, memberId]);
            if (existingVote)
                return false;
            // Check if election is in voting phase
            const election = await (0, database_1.executeQuerySingle)('SELECT election_status FROM leadership_elections WHERE id = ?', [electionId]);
            return election?.election_status === 'Voting Open';
        }
        catch (error) {
            throw (0, errorHandler_1.createDatabaseError)('Failed to check voting eligibility', error);
        }
    }
    // Get elections count
    static async getElectionsCount(filters = {}) {
        try {
            let whereClause = 'WHERE 1=1';
            const params = [];
            if (filters.hierarchy_level) {
            ...
            throw (0, errorHandler_1.createDatabaseError)('Failed to get elections count', error);
        }
    }
}
