  ...
  try {
    ...
    if (existingPermission.length === 0) {
      const [result] = await connection.execute(
        ...
        ['financial.view_trends', 'View financial trends and analytics', 'financial']
      );
    ...
    // 5. Also assign to membership_approver role if it exists
    const [membershipRole] = await connection.execute(
      'SELECT id FROM roles WHERE name = ?',
      ['membership_approver']
    );
    ...
    if (membershipRole.length > 0) {
      ...
      const [existingMembershipPermission] = await connection.execute(
        'SELECT id FROM role_permissions WHERE role_id = ? AND permission_id = ?',
        [membershipRoleId, permissionId]
      );

      if (existingMembershipPermission.length === 0) {
        await connection.execute(
          'INSERT INTO role_permissions (role_id, permission_id) VALUES (?, ?)',
          [membershipRoleId, permissionId]
        );
        console.log('✅ Assigned financial.view_trends permission to membership_approver role');
      } else {
        console.log('✅ membership_approver role already has financial.view_trends permission');
      }
    }
